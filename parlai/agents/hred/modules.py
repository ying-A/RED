...
"""Module files as torch.nn.Module subclasses for HredAgent."""
...
def opt_to_kwargs(opt):
    ...
    kwargs = {}
    for k in ['numlayers','hier_numlayers', 'dropout', 'bidirectional','hier_bidirectional','rnn_class',
              'lookuptable', 'decoder', 'numsoftmax',
              'attention', 'attention_length', 'attention_time',
              'input_dropout','dli_input_dim','dli_rnn_hiddensize','dli_ffn_dimension','max_turns','max_single_seq_len']:
        if k in opt:
            kwargs[k] = opt[k]
    return kwargs
...
class Hred(TorchGeneratorModel):
    ...
    def __init__(
        self, order_choice, num_features, embeddingsize, hiddensize,hier_hiddensize, numlayers=2,hier_numlayers=2,dropout=0,
        bidirectional=False, hier_bidirectional=False, rnn_class='gru', lookuptable='unique',
        ...
        dli_rnn_hiddensize=64,dli_ffn_dimension=128,max_turns=30,max_single_seq_len=50
    ):
        ...
        shared_rnn = None
        shared_hier_rnn = None

        self.encoder = HredEncoder(
            num_features, embeddingsize, hiddensize,hier_hiddensize,
            padding_idx=padding_idx, rnn_class=rnn_class,
            numlayers=numlayers, hier_numlayers=hier_numlayers,dropout=dropout,
            bidirectional=bidirectional,hier_bidirectional=hier_bidirectional,
            shared_lt=shared_lt, shared_rnn=shared_rnn,shared_hier_rnn=shared_hier_rnn,
            unknown_idx=unknown_idx, input_dropout=input_dropout,max_turns=max_turns,max_single_seq_len=max_single_seq_len)
...
class HredEncoder(nn.Module):
    ...
    def __init__(self, num_features, embeddingsize, hiddensize,hier_hiddensize,
                 padding_idx=0, rnn_class='gru', numlayers=2, hier_numlayers=2,dropout=0.1,
                 bidirectional=False,hier_bidirectional=False, shared_lt=None, shared_rnn=None,
                 shared_hier_rnn=None,input_dropout=0, unknown_idx=None, sparse=False,max_turns=30,
        ...
        self.hier_layers = hier_numlayers
        self.hier_dirs = 2 if hier_bidirectional else 1
        self.hier_hsz = hier_hiddensize
        ...
        if shared_rnn is None and shared_hier_rnn is None:
            self.rnn = rnn_class(embeddingsize, hiddensize, numlayers,
                                 dropout=dropout if numlayers > 1 else 0,
                                 batch_first=True, bidirectional=bidirectional)
            self.hier_rnn = rnn_class(hiddensize, hier_hiddensize, hier_numlayers,
                                 dropout=dropout if numlayers > 1 else 0,
                                 batch_first=True, bidirectional=hier_bidirectional)
        elif bidirectional:
            raise RuntimeError('Cannot share decoder with bidir encoder.')
        else:
            self.rnn = shared_rnn
            self.hier_rnn = shared_hier_rnn
    ...
    def forward(self, input, his_turn_end_ids):
        ...
        his_encoder_outs = his_encoder_outs.view(bsz,self.max_turns,real_max_seq_len,self.hsz)
    
        expand_mask = mask.unsqueeze(-1).expand(bsz,self.max_turns,real_max_seq_len*self.hsz)
        expand_mask = expand_mask.view(bsz,self.max_turns,real_max_seq_len,self.hsz)
        ...
        sorted_hier_xes, sorted_in_example_ids,sorted_hier_x_lens = argsort(
            hier_x_lens,hier_xes,in_example_ids,hier_x_lens,descending=True)
        
        hier_xes_packed = pack_padded_sequence(sorted_hier_xes, sorted_hier_x_lens, batch_first=True)
        
        hier_out_packed,hier_hidden_packed = self.hier_rnn(hier_xes_packed)
        hier_out_padded,_ = pad_packed_sequence(hier_out_packed, batch_first=True)

        hier_after_sort_idxs = torch.LongTensor(argsort(sorted_in_example_ids,in_example_ids,descending=False)[0]).cuda()
        hier_his_encoder_outs = torch.index_select(hier_out_padded,0,hier_after_sort_idxs)

        real_max_his_n_turn = hier_his_encoder_outs.size(1)
        hier_final_encoder_outs = hier_his_encoder_outs.view(bsz,real_max_his_n_turn,-1)
        transpose_hidden = _transpose_hidden_state(hier_hidden_packed)

        hier_fianl_hidden = torch.index_select(transpose_hidden,0,hier_after_sort_idxs)
        ...
        if self.hier_dirs > 1:
            ...
            if isinstance(self.hier_rnn, nn.LSTM):
                hier_fianl_hidden = (hier_fianl_hidden[0].view(-1, self.hier_dirs, bsz, self.hier_hsz).sum(1),
                                     hier_fianl_hidden[1].view(-1, self.hier_dirs, bsz, self.hier_hsz).sum(1))
            else:
                hier_fianl_hidden = hier_fianl_hidden.view(-1, self.hier_dirs, bsz, self.hier_hsz).sum(1)
...
class RNNDecoder(nn.Module):
    ...
    def __init__(self, num_features, embeddingsize, hiddensize,
                 padding_idx=0, rnn_class='gru', numlayers=2, dropout=0.1,

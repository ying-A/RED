...
class Batch(AttrDict):
    ...
    def __init__(self, text_vec=None, text_lengths=None,
                 ...
                 image=None, memory_vecs=None, observations=None,his_turn_end_ids=None,
                 **kwargs):
        super().__init__(
            ...
            image=image, memory_vecs=memory_vecs, observations=observations,his_turn_end_ids=his_turn_end_ids,
            **kwargs)
...
class TorchAgent(Agent):
    ...
    def _set_text_vec(self, obs, truncate, split_lines,his_split):
        ...
        if 'text_vec' in obs:
            ...
            if split_lines and not his_split:
            ...
            elif split_lines and his_split:
                his_turn_vecs = []
                flatten_his_ves = []
                his_turn_end_ids = []
                end_id = -1
                for line in obs['text'].split('\n'):
                    his_turn_vecs.append(
                        self._vectorize_text(line, truncate=truncate))
                for sing_turn in his_turn_vecs:
                    for wd in sing_turn:
                        flatten_his_ves.append(wd)
                    end_id += len(sing_turn)
                    his_turn_end_ids.append(end_id)
                obs['text_vec'] = torch.LongTensor(flatten_his_ves)
                obs['his_turn_end_ids'] = torch.LongTensor(his_turn_end_ids)
    ...
    def vectorize(self, obs, add_start=True, add_end=True, split_lines=False,
        ...
        :param his_split:   If set True, split history turns and flatten and 
                            get the length of each history turn

        :return: the input observation, with 'text_vec', 'label_vec', and
            'cands_vec' fields added.
        """
        split_lines = True
        self._set_text_vec(obs, text_truncate, split_lines,his_split=True)
    ...
    def batchify(self, obs_batch, sort=False,
        ...
        if any('text_vec' in ex for ex in exs):
            ...
            hids = [ex.get('his_turn_end_ids', self.EMPTY).cuda() for ex in exs]

            xs, x_lens = padded_tensor(_xs, self.NULL_IDX, self.use_cuda)
            if sort:
                sort = False  # now we won't sort on labels
                xs, x_lens, valid_inds, exs,hids = argsort(
                    x_lens, xs, x_lens, valid_inds, exs,hids, descending=True
                )
            # for hid in hids:
            #     hid = hid.cuda()
        ...
        return Batch(text_vec=xs, text_lengths=x_lens, label_vec=ys,
                     ...
                     observations=exs,his_turn_end_ids=hids)

...
class TorchGeneratorModel(nn.Module):
    ...
    def forward(self, xs,his_turn_end_ids, ys=None, cand_params=None, prev_enc=None, maxlen=None):
        """Get output predictions from the model.

        :param xs: input to the encoder
        :type xs: LongTensor[bsz, seqlen]
        :param his_turn_end_ids: 
        :type his_turn_end_ids: LongTensor[bsz, turnlen]
        ...
        if self.model_name =="hred":
            encoder_states = prev_enc if prev_enc is not None else self.encoder(xs,his_turn_end_ids)
            dliloss = self.dli(encoder_states[0],encoder_states[2])
        elif self.model_name == "recosa":
            encoder_states = prev_enc if prev_enc is not None else self.encoder(xs,his_turn_end_ids)
            dliloss = self.dli(encoder_states[0],encoder_states[1])
        else:
            encoder_states = prev_enc if prev_enc is not None else self.encoder(xs)
            dliloss = self.dli(encoder_states[0],his_turn_end_ids)

            # # Code Used For Plot transformer TSNE Embedding. # # 
            # # The following codes can be used to get the embeddings of history utterances in the evaluation step.
            # import pickle
            # tmp = {}
            # tmp['xs']= xs
            # tmp['enc'] = encoder_states[0]
            # tmp['his_turn_end_ids'] = his_turn_end_ids
            # with open("transformer_analysis","ab") as f:
            #     pickle.dump(tmp,f)
        ...
        return scores, preds, encoder_states, dliloss
...
class TorchGeneratorAgent(TorchAgent):
    ...
    def _init_cuda_buffer(self, batchsize, maxlen, his_max_turn,force=False):
        ...
        if self.use_cuda and (force or not hasattr(self, 'buffer_initialized')):
            try:
                ...
                dummy_his = torch.ones(batchsize, his_max_turn).long().cuda()
                dummy_his = torch.cumsum(dummy_his,-1)
                dummy_ys = torch.ones(batchsize, 2).long().cuda()
                scores, _, _,dliloss = self.model(dummy_xs, dummy_his,dummy_ys)
                ppl_loss = self.criterion(
                    scores.view(-1, scores.size(-1)), dummy_ys.view(-1)
                )
                loss = ppl_loss + dliloss
                loss.backward()
    ...
    def reset_metrics(self):
        ...
        super().reset_metrics()
        self.metrics['ppl_loss'] = 0.0
        self.metrics['num_tokens'] = 0
        self.metrics['correct_tokens'] = 0
        self.metrics['dli_loss'] = 0.0
    ...
    def report(self):
        ...
        if num_tok > 0:
            ...
            m['dli_loss'] = self.metrics['dli_loss'] / num_tok
            m['ppl_loss'] = self.metrics['ppl_loss'] / num_tok
            m['loss'] = m['dli_loss'] + m['ppl_loss']
            try:
                m['ppl'] = math.exp(m['ppl_loss'])
    ...
    def train_step(self, batch):
        ...
        self._init_cuda_buffer(batchsize, self.truncate or 256,30) # max_turn:30
        self.model.train()
        self.zero_grad()

        try:
            
            scores, preds, _,dliloss = self.model(batch.text_vec, batch.his_turn_end_ids,batch.label_vec)

            score_view = scores.view(-1, scores.size(-1))
            ppl_loss = self.criterion(score_view, batch.label_vec.view(-1))
            ...
            self.metrics['ppl_loss'] += ppl_loss.item()
            self.metrics['num_tokens'] += target_tokens
            self.metrics['dli_loss'] += dliloss.item()*target_tokens
            ppl_loss /= target_tokens  # average loss per token
            loss = ppl_loss + dliloss
            loss.backward()
            self.update_params()
        except RuntimeError as e:
    ...
    def eval_step(self, batch):
        ...
        if self.model.model_name =="recosa" or "transformer":

            split_batches = []
            split_batches_Batch = []
            for i in range(bsz//per_max_num_bsz):
                d = {}
                for x in batch:
                    if batch[x] is not None:
                        d[x] = batch[x][i*per_max_num_bsz:(i+1)*per_max_num_bsz]
                    else:
                        d[x] = None
                split_batches.append(d)
            last_start = bsz//per_max_num_bsz*per_max_num_bsz
            if bsz%per_max_num_bsz !=0:
                d = {}
                for x in batch:
                    if batch[x] is not None:
                        d[x] = batch[x][last_start:]
                    else:
                        d[x] = None
                split_batches.append(d)
        
            for s_b in split_batches:
                split_batches_Batch.append(
                    Batch(text_vec=s_b['text_vec'], text_lengths=s_b['text_lengths'], label_vec=s_b['label_vec'],
                            label_lengths=s_b['label_lengths'], labels=s_b['labels'],
                            valid_indices=s_b['valid_indices'], candidates=s_b['candidates'],
                            candidate_vecs=s_b['candidate_vecs'], image=s_b['image'], memory_vecs=s_b['memory_vecs'],
                            observations=s_b['observations'],his_turn_end_ids=s_b['his_turn_end_ids'])
                )
            all_preds = []
            ALL_SP_PREDS = []
            for batch in split_batches_Batch:
                if self.skip_generation:
                    warn_once(
                        "--skip-generation does not produce accurate metrics beyond ppl",
                        RuntimeWarning
                    )
                    logits, preds, _,dliloss = self.model(batch.text_vec,batch.his_turn_end_ids, batch.label_vec)
                elif self.beam_size == 1:
                    # greedy decode
                    logits, preds, _,dliloss = self.model(batch.text_vec,batch.his_turn_end_ids)
                elif self.beam_size > 1:
                ...
                if batch.label_vec is not None:
                    ...
                    f_scores, f_preds, _,f_dliloss = self.model(batch.text_vec, batch.his_turn_end_ids,batch.label_vec)
                    ...
                    self.metrics['correct_tokens'] += correct
                    self.metrics['ppl_loss'] += ppl_loss.item()
                    self.metrics['dli_loss'] += f_dliloss.item()*target_tokens
                    self.metrics['num_tokens'] += target_tokens
                    self.metrics['loss'] += (ppl_loss.item() + f_dliloss.item()*target_tokens)
                    self.metrics['this_example_ppl_loss'] = ppl_loss.item()
                    self.metrics['this_example_num_tokens'] = target_tokens
                    self.metrics['this_example_his_turns_num'] = len(batch.his_turn_end_ids[0])
                all_preds.append(preds)
            for s_p in all_preds:
                for p in s_p:
                    ALL_SP_PREDS.append(p)
        else:
            if self.skip_generation:
                warn_once(
                    "--skip-generation does not produce accurate metrics beyond ppl",
        ...
        text = [self._v2t(p) for p in ALL_SP_PREDS]
...
class PerplexityEvaluatorAgent(TorchGeneratorAgent):
    ...
    def next_word_probability(self, partial_out):
        ...
        his_turn_end_ids = obs['his_turn_end_ids'].unsqueeze(0)
